...
{
    public static class get_ingredients
    {
        [FunctionName("get_ingredients")]
            ...
            ILogger log)
        {
            ...
            try
            {
                ...
                var ingredientsQuery = database.GetCollection<Ingredient>("ingredients").FindAsync(x => true);
                var ingredients = await ingredientsQuery;
                return (ActionResult)new OkObjectResult(ingredients.ToList());
            }
            catch (Exception ex)
            {
